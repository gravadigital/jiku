...
- **CallerClass / ExternalScopeSpec** (`core/src/queries/types.ts`) - What the service clips for a caller, and the resource's declaration of the external-mode clip. Declaring the clip IS applying it: there is no boolean, so the dangerous state is unrepresentable. Both of its names are database columns, not contract fields.
