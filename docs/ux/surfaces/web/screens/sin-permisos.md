...
- **[REQ-013] La pantalla enteramente en estilos inline entra en la migración.** Es la única del producto sin módulo SCSS: los cuatro bloques llevan estilos inline, incluido un `backgroundColor: '#e91e8c'` que no existe en ninguna paleta —ni siquiera en la vieja— y un `color: '#666'` escrito literal en vez del token. Con modo oscuro esos valores hardcodeados quedarían visiblemente rotos, porque no responden a la redeclaración de tokens bajo `[data-theme='dark']`. La pantalla pasa a CSS Modules con tokens semánticos, como el resto. **El microcopy no cambia acá**: que el mensaje no diga a dónde ir ni mencione el portal del cliente sigue registrado en `gaps-as-is.md` y pertenece a FG-5 [REQ-013 RF-2, CA-2, §Fuera de Alcance].
