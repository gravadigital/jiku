...
| 39 | card-horas-trabajadas | card | — | content | desktop | todos los estados | Desglose por persona de las horas imputadas al requisito, con el total al pie. Segunda card de la columna derecha |
| 40 | total-horas-trabajadas | heading | h3 | content | desktop | hidden_in_states: horas cargando, horas no disponibles | Total de horas del requisito, incluidas las de sus tareas — **última fila de la card**, bajo el desglose |
...
`card-estado`, `seccion-tareas`, `acordeon-campo`, `formulario-comentario` y `card-resolucion` se relevaron como `card` / `section`: son compuestos sin tipo propio en el diccionario. `stepper-workflow` se relevó como `progress-bar` por ser lo más cercano del diccionario: no es una barra, son 5 nodos con conectores y un símbolo por nodo (`✓`, `×` o el número). **Desde REQ-013 esa aproximación tiene destino concreto:** el DS especifica el componente `Stepper` (`components/stepper.md`), que es exactamente este control, así que el bloque se implementa con él. El tipo de bloque de esta tabla sigue siendo `progress-bar` porque es lo que el diccionario de bloques ofrece —la carencia es del diccionario, no del Design System— y cambiarlo acá sin que el diccionario lo soporte haría que el generador lo dibuje con el fallback genérico. El chrome es compartido; los pills-dropdown están documentados aparte [fuente: código-existente].
...
- row `dos-columnas`
  ...
  - col ~5/12 (420px fijos): columna derecha
    ...
    - card-horas-trabajadas (desglose-horas-persona, total-horas-trabajadas, vacio-horas-trabajadas *(sin horas)*, cargando-horas-trabajadas *(mientras carga)*) [REQ-010 RF-8]
...
**El card de horas va segundo en la columna derecha, entre "Información General" y "Etiquetas"** (`RequirementDetail.tsx:340-348`) [fuente: código-existente]. El criterio implementado es que las horas son **el dato de consulta que se lee junto a los datos de identificación del requisito**: quien abre el detalle para decidir qué hacer con él mira proyecto, responsables y esfuerzo invertido en la misma pasada, sin recorrer la columna entera. Etiquetas y resolución quedan después, agrupadas como lo que son —clasificación y cierre.

> **Esto revisa la decisión que REQ-010 RF-8 había tomado.** El requerimiento había ubicado la card **última**, con el argumento de que intercalarla partiría en dos una secuencia de identificación-y-cierre formada por contexto, etiquetas y resolución. La implementación la ubicó segunda y el criterio que prevaleció es el de arriba: la afinidad de las horas es con la identificación del requisito, no con su cierre, así que la secuencia que se preserva es otra. **La documentación se corrige contra el código, no al revés** — la card está en producción en esa posición. El argumento original queda registrado acá para que no se lo redescubra como si nunca se hubiera discutido.
...
**Comportamiento observado a ≤1023px** (fuera de los viewports declarados de la superficie): el encabezado se apila en columna (titulo-requisito, fila de badges, boton-volver + boton-editar) y el cuerpo pasa a columna única en el orden del DOM: card-contexto, card-estado, seccion-tareas, card-actividad, card-informacion-general, card-horas-trabajadas, card-etiquetas, card-resolucion. En una columna la resolución queda al final, después del feed de actividad y del formulario de comentario, y la información general —con las horas pegadas debajo— después de las tareas. Los dos módulos llevan un comentario de código que explica el porqué —tablet unificado con mobile por "pedido explícito del usuario", y el apilado del header para que los botones de ancho fijo no corten el título— (`RequirementDetail.module.scss:9-14`, `RequirementHeader.module.scss:7-13`) [fuente: código-existente].
...
- Icono: nada
- Asset: nada
- Annotation: card de la columna derecha, **segunda**, entre `"Información General"` y `"Etiquetas"` (`RequirementDetail.tsx:340-348`). El cuerpo es una `<dl>` con **el desglose por persona primero y el total como última fila**, separado por una regla superior y con el label `"Total"` destacado: se lee de arriba hacia abajo como una suma, no como un total que después se descompone. Se carga por su cuenta, con una consulta propia y no con el payload del requisito: **si falla o tarda, el resto del detalle se renderiza igual** [REQ-010 RF-8, Escenario B]

### total-horas-trabajadas
- Texto/label: el total formateado como `Xh Ym` — `"5h 0m"` para 300 minutos
- Icono: nada
- Asset: nada
- Annotation: **va al pie de la card, como última fila del desglose**, con una regla que lo separa de las filas por persona y el label `"Total"` destacado (`RequirementWorkedHoursCard.tsx:47-50`). Suma las horas imputadas directamente al requisito **más** las de sus tareas, sin doble conteo. Es el mismo número que muestra la columna `"Hs. Trab."` del listado, y siempre es igual a la suma de las filas de arriba [REQ-010 RF-3, AC-6, AC-10]
...
- **El total se muestra además del desglose, aunque sea la suma de las filas de arriba.** La redundancia es deliberada: la pregunta más frecuente es "cuánto lleva", y obligar a sumar mentalmente cinco filas para responderla sería peor que repetir un número [REQ-010 RF-8, AC-10]. **La implementación lo ubicó al pie y no al encabezado de la card**, separado por una regla: la card se lee como una suma que cierra, no como un total que después se justifica.
...
- **La card va segunda en la columna derecha, debajo de "Información General" — y esto revisa lo que REQ-010 RF-8 había decidido.** El requerimiento la había puesto **última**, razonando que contexto, etiquetas y resolución forman una secuencia de identificación y cierre a la que el dato de horas no pertenece, y que al final no competiría con los botones de resolver y cancelar. La implementación la ubicó segunda con un criterio distinto y es el que rige: **las horas se leen junto a los datos de identificación del requisito** —proyecto, responsables, esfuerzo invertido en una sola pasada— porque es ahí donde se decide qué hacer con él; etiquetas y resolución quedan después, agrupadas como clasificación y cierre. La objeción original sobre las acciones de resolución **no se materializó**: la card es de solo lectura y no agrega paradas al orden de foco, así que no compite con nada. Queda registrado que fue una decisión revisada y no una omisión [REQ-010 RF-8; `RequirementDetail.tsx:340-348`].
...
- **[REQ-013] El corte de 1023px se unifica con el 1024px de las dos pantallas de formulario.** Esta pantalla colapsa a una columna en `max-width: 1023px` mientras `alta-requisito` y `edicion-requisito` lo hacen en `max-width: 1024px`: es un píxel de diferencia entre pantallas hermanas del mismo dominio, y el CHANGELOG del DS lo marca como **error puntual**, no como avance parcial del mobile pendiente. Se unifican en un único valor. **El resto de la deuda de breakpoints no se toca** —el 1200px de `detalle-proyecto`, el 1680px de las vistas agregadas, el shell sin media queries— y sigue en FG-5: corregir un error de un píxel no es lo mismo que decidir cómo se ve Jiku en un teléfono [REQ-013 RF-9, CA-15].
- **[REQ-013] Los tres controles de estado conservan su reparto.** El stepper informa los cinco pasos de trabajo, el badge editable del encabezado ofrece los siete estados y la card de resolución cierra y reabre. El DS especifica los tres por separado y declara explícitamente que **coincide con lo que el código ya hace** desde S-050: la migración cambia cómo se ven, no qué hacen. Es la preservación de comportamiento más sensible del REQ, porque tres controles sobre un mismo dato son fáciles de "simplificar" por error al rehacerlos [REQ-013 RF-10, CA-17].
