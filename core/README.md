...
| `OPUS_URL`                                  | absolute base URL **with scheme** of the customer portal (e.g. `https://opus.example.com`), used to build the link of a notification when it is enqueued (REQ-015/S-071). **This is NOT `OPUS_DOMAIN`** (below): `OPUS_DOMAIN` is the ingress host, consumed as `https://${OPUS_DOMAIN}` in `docker-compose.yml`; `OPUS_URL` is the absolute base with scheme. **Core fails to start without it** — no default, on purpose: any default would send every mail's link to the wrong domain, and a broken link in an already-sent mail cannot be fixed afterwards. |
| `OPUS_DOMAIN`                               | the ingress host (no scheme). Declared in `deploy/.env.dist`, consumed by the reverse proxy — not read by `core` directly. See `OPUS_URL` above for the difference. |
