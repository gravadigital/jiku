...
| `OPUS_URL`                                  | absolute base URL **with scheme** of the customer portal (e.g. `https://opus.example.com`), used to build the link of a notification when it is enqueued (REQ-015/S-071). **Core fails to start without it** — no default, on purpose: any default would send every mail's link to the wrong domain, and a broken link in an already-sent mail cannot be fixed afterwards. **It is not declared in the `.env`**: the compose builds it from `OPUS_DOMAIN` (below) — `https://${OPUS_DOMAIN}` in production, a fixed `http://localhost:<port>` in dev and local, where there is no ingress. |
| `OPUS_DOMAIN`                               | the portal host, **no scheme** (e.g. `opus.example.com`). The only one of the two an operator declares, in `deploy/.env.dist`. `core` never reads it: the ingress consumes it bare, and `docker-compose.yml` adds the scheme to produce `OPUS_URL` above. Putting a scheme here yields `https://https://…` in every mail link. |
| `SMTP_HOST`                                 | SMTP server host (REQ-015/S-073) — core's first outbound network dependency. Read **lazily**, on the first dispatch cycle that has a row to send, not at startup: a misconfigured SMTP does not corrupt data, it leaves rows `pending` with `last_error`. |
| `SMTP_PORT`                                 | SMTP port, default `587`.                                                                                                                                                                                                              |
| `SMTP_USER`                                 | SMTP auth user.                                                                                                                                                                                                                          |
| `SMTP_PASSWORD`                             | SMTP auth password. **Secret** — empty in `deploy/.env.dist`, never a placeholder that looks valid.                                                                                                                                     |
| `SMTP_FROM`                                 | sender address of outgoing notification mails.                                                                                                                                                                                           |
